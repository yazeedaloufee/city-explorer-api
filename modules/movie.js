...
let handleMovie = async (req, res) => {
    ...
    if(movieCache[query]!==undefined){
      res.send( movieCache[query])
      console.log('cach hit, data has been revrieved from cache');
   ...
   movieCache[query]=moviesInfo;
        console.log("miss cache, data has just been added to cache");
    res.send(moviesInfo);
}}
